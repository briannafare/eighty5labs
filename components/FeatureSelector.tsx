...
const CapabilityTile = ({ title, icon: Icon, color, desc }: { title: string, icon: React.ComponentType, color: string, desc: string }) => (
  <div className="group cursor-pointer hover-lift">
    <div className="h-40 rounded-t-[12px] border border-black/[0.08] border-b-0 bg-white/60 backdrop-blur-sm relative overflow-hidden transition-all group-hover:bg-white/80">
       <div className="absolute inset-x-0 top-0 h-10 border-b border-black/[0.03] bg-black/[0.01] flex items-center px-4">
          <div className="flex gap-1.5">
             <div className="w-1.5 h-1.5 rounded-full bg-black/10"></div>
             <div className="w-1.5 h-1.5 rounded-full bg-black/10"></div>
          </div>
       </div>
       <div className="absolute inset-0 mt-10 flex flex-col items-center justify-center p-4">
          <div className="w-14 h-14 rounded-[12px] border border-black/[0.05] bg-white flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform mb-2 text-black/80">
             <Icon />
          </div>
          <span className="text-[10px] text-zinc-500 font-black uppercase tracking-widest px-2 py-0.5 border border-black/5 bg-white/50 rounded-full">{color}</span>
       </div>
    </div>
    <div className="p-6 border border-black/[0.08] rounded-b-[12px] bg-white group-hover:border-black/20 transition-all min-h-[110px] shadow-sm">
      <div className="text-[15px] font-bold text-black mb-2 tracking-tight group-hover:text-black/80">{title}</div>
    ...
    <div className="container-custom">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-10 relative">
         <div className="max-w-xl text-left">
            ...
            <h2 className="text-4xl font-[900] tracking-tighter text-black mb-4">Autonomous Marketing Stack</h2>
            <p className="text-zinc-600 text-lg font-medium leading-relaxed tracking-tight">Everything your team does manually, our agents do instantly and accurately, 24/7.</p>
         </div>
